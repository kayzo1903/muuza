...
import { Star, MapPin, Clock, Utensils, Share2, MessageCircle, ShoppingCart, X, Bell, Plus } from "lucide-react";
...
export default function ChefProfile() {
  ...
  const menuItems = [
    ...
    { 
      ...
      description: "Fragrant rice dish with marinated meat"
    }
  ];
  ...
  const toggleNotifications = () => {
    ...
    toast.success(
      notificationsEnabled 
        ? "Notifications disabled for this chef" 
        : "Notifications enabled for this chef"
    );
  };
      ...
      <div className="px-4 md:px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6 py-4 border-t">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>{chef.openingHours}</span>
          </div>
          <div className="flex items-center gap-2">
            <Utensils className="w-5 h-5 text-primary" />
            <span>{chef.cuisine.join(", ")}</span>
          </div>
        </div>
